...
def compute_actual_lens(args, attn_mask, device): # compute num of positions need to drop, and tag the position
    ...
    drop_nums = (args.p_rate * actual_len).long()  # [B,]  ---> [B, H, L, L]
...
def compute_integrated_gradient(args, model, input_ids, labels, extend_attn_mask_list_A, layer, segment_ids):
    model.eval()
...
def compute_attention_attribution(loss, attention, attribution, is_abs_attr=False):
    #to be fix: complete IGA attr
    attention.retain_grad()
    if attribution == 'AA':
        attr = attention  # directly use attention weights for attribution
    elif attribution == 'GA':
        (attr,) = torch.autograd.grad(loss, attention, retain_graph=True)
    if is_abs_attr:
        attr = torch.abs(attr)  
    return attr   

def generate_mask_by_attribution(args, attr_matrix, extend_attn_mask, attention_mask, device):
    # masking low or high
    grad_masks = None
    if args.attribution == 'RD':
        grad_masks = generate_mask_random(args, extend_attn_mask, device)
    elif args.drop_direction == 'low':
        grad_masks = generate_mask_low(args, attr_matrix, extend_attn_mask, attention_mask,
                                        device)  # [B, H, L, L]
    elif args.drop_direction == 'high':
        grad_masks = generate_mask_high(args, attr_matrix, extend_attn_mask, attention_mask,
                                        device)  # [B, H, L, L]
    else:
        raise NotImplementedError
    return grad_masks